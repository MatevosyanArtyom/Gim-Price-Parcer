...
{
    public class XlsxTemplateNotik
    {
...
foreach (var sheet in sheets)
{
    ...
    for (var i = 2; i < 5; i++)
    {
        ...
        Func<string, decimal?> tryParse = s =>
            decimal.TryParse(s.Where(c => char.IsDigit(c) || char.IsSeparator(c)).ToString(),NumberStyles.AllowDecimalPoint, format, out var price)
                ? price
                : (decimal?) null;

        var price1 = tryParse(sheet.GetValue<string>(i, 5));
        var price2 = tryParse(sheet.GetValue<string>(i, 6));
        ...
        var item = new PriceListItemSource
        {
            ...
            ProductName = sheet.GetValue<string>(i, 3),
            Price1 = price1 ?? 0,
            Price2 = price2,
